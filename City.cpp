...
*/
void CCity::OnDraw(Gdiplus::Graphics *graphics, int offsetX, int offsetY)
{
...
/** \brief Add a blank tile to the city
* \param blankTile New blank tile to add
*/
void CCity::AddBlank(std::shared_ptr<CBlankTile> blankTile)
{
	mBlankTiles.push_back(blankTile);
}
...
void CCity::XmlTile(const std::shared_ptr<CXmlNode> &node)
{
	...
	{
		tile = make_shared<CTilePower>(this);
	}
...
/** Testing whether this tile match with any blank tiles
* \param tile
*/
bool CCity::MatchTest(std::shared_ptr<CTile> tile)
{
	bool match = false;
	for (auto blanktile : mBlankTiles)
	{
		if ((blanktile->GetX()) == (tile->GetX()) && (blanktile->GetY()) == (tile->GetY())
			&& (blanktile->GetFile()) == (tile->GetFile()))
		{
			match = true;
			auto loc = find(::begin(mBlankTiles), ::end(mBlankTiles), blanktile);
			if (loc != ::end(mBlankTiles))
			{
				mBlankTiles.erase(loc);
			}
			break;
		}
	}
