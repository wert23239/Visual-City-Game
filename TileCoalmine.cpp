...
/// The ratio at which trumping alters the speed of production
const double TrumpSpeed = 2;
...
void CTileCoalmine::Update(double elapsed)
{
	...
	if (!mIsDestroyed && mUpdate)
	{
		...
		if (mProduction == LowProduction && mDuration >= MediumProductionTime)
		{
			...
			SetImage(L"coalmine-med.png");
			mDuration = 0;
		}

		if (mProduction == MediumProduction && mDuration >= FullProductionTime)
		{
			mProduction = FullProduction;
			SetImage(L"coalmine-full.png");
			mDuration = 0;
		}
	}
}
...
*/
double CTileCoalmine::CountCoal()
{
	//temporary coal variable to return
	double currentCoal = mProduction;
